...
async def start_session(mode:str = "interact"):
    ...
    if mode.strip().lower() == "interact":
        session_manager.sessions[session_id]["llm"] = llm.llm(system_prompt_for_interact)
    ...
    while not session['action_done'] and session['retry_count'] < MAX_RETRIES:
        try:
            ...
            if session['retry_count'] >= MAX_RETRIES:
                return {"status": "failure", "error": "Element type not found after max retries", "commands_executed": session['commands_executed']}
